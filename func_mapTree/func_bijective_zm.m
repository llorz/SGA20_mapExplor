function [T12, T21, C12, C21] = func_bijective_zm(S1, S2, T12_ini, T21_ini, para)

s1_id = S1.samples;
s2_id = S2.samples;

C12_ini = fMAP.pMap2fMap(S2.evecs(:,1:para.k_init), S1.evecs(:, 1:para.k_init), T21_ini);
C21_ini = fMAP.pMap2fMap(S1.evecs(:,1:para.k_init), S2.evecs(:, 1:para.k_init), T12_ini);
...
C12 = fMAP.pMap2fMap(S2.evecs(:,1:para.k_final), S1.evecs(:, 1:para.k_final), T21);
C21 = fMAP.pMap2fMap(S1.evecs(:,1:para.k_final), S2.evecs(:, 1:para.k_final), T12);
