function [all_T12, all_T21] = find_mulitple_maps_shape_pair(S1, S2, para)
...
S2 = mesh_with_consistent_fields(S2);
S2 = MESH.compute_LaplacianBasis(S2, para.num_eigs);
S2.repeatedID = get_repeated_eigenvalues(S2.evals, para.thres_repeating_eigs);
S2.samples = MESH.get_samples(S2, para.num_samples_on_shape, 'euclidean');
...
all_T21 = cellfun(@(C12) fMAP.fMap2pMap(S1.evecs, S2.evecs,C12),all_C12_refined,'un',0);
all_T12 = cellfun(@(C12) fMAP.fMap2pMap(S2.evecs, S1.evecs,C12'),all_C12_refined,'un',0);
end